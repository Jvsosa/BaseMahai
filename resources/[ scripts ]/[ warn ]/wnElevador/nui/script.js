$(function() {
  ...
  function init(btns) {
	$(".container-elevador").empty();
	(Object.entries(btns)).forEach(v => {
		$(".container-elevador").prepend(`
			<button class="menuoption" data-action="${v[0]}"><po>${v[1].title}</po></button>
		`);
	});
  ...
  function sendData(name, data) {
	$.post("http://wnElevador/" + name, JSON.stringify(data), function(
	  datab
	) {
	  if (datab != "ok") {
		console.log(datab);
