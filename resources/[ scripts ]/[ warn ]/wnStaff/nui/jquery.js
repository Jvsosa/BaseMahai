...
function listTeleport(){
    $.post("http://wnStaff/CoordsLista",JSON.stringify({}),(data) => {
...
function createCoord(){
	...
	if($el.length) {
		$.post("http://wnStaff/addteleport",JSON.stringify({
			nome:nome
		}), (data) =>{ 
...
function teleportPlayer(data) {
	$.post("https://wnStaff/teleport",JSON.stringify({name:data.dataset.index,x:data.dataset.x,y:data.dataset.y,z:data.dataset.z}))
}

function deleteTeleport(data) {
	$.post("https://wnStaff/deleteteleport",JSON.stringify({
		id : data.dataset.id,
...
function listLogs(){
    $.post("http://wnStaff/LogsLista",JSON.stringify({}),(data) => {
...
function listaControle(){
    $.post("http://wnStaff/ControleLista",JSON.stringify({}),(data) => {
...
function enviarIDPlayer(data) {
	if (data.dataset.passaporte !== '3' && data.dataset.passaporte !== '7' && data.dataset.passaporte !== '4' && data.dataset.passaporte !== '2') {
	  $("#nPlayer-VerPerfil").html(data.dataset.nome);
  
	  $.post("https://wnStaff/EnviarID", JSON.stringify({
...
function createAviso(){
	...
	if($el.length) {
		$.post("http://wnStaff/addAviso",JSON.stringify({
			motivo:motivo
		}), (data) =>{ 
			if(data.retorno == 'done') {
				closeAviso()
...
function PunicoesLista(){
    $.post("http://wnStaff/PunicoesLista",JSON.stringify({}),(data) => {
...
function DeleteAdv(data) {
	$.post("https://wnStaff/DeleteAdv",JSON.stringify({
		passaporte : data.dataset.passaporte,
		ide : data.dataset.id,
...
function ItensLista(){
    $.post("http://wnStaff/ItensLista",JSON.stringify({}),(data) => {
...
function FormatConfirmItem(data) {
	...
	$.post("https://wnStaff/enviarItem",JSON.stringify({
...
function PegarItemConfirmar(){
	...
	if($el.length) {
		$.post("http://wnStaff/pegarItemConfirm",JSON.stringify({
...
function AnunciosLogs(){
    $.post("http://wnStaff/AnunciosLogs",JSON.stringify({}),(data) => {
...
function GaragemLista(){
    $.post("http://wnStaff/garagemLista",JSON.stringify({}),(data) => {
...
function FormatConfirmGaragem(data) {
	...
	$.post("https://wnStaff/enviarCarro",JSON.stringify({
...
function PegarGaragemConfirmar(){
	...
	if($el.length) {
		$.post("http://wnStaff/pegarCarroConfirmar",JSON.stringify({
...
function AnunciosLista(){
    $.post("http://wnStaff/AnunciarLista",JSON.stringify({}),(data) => {
...
function EnviarAnuncioInformacoes(data) {
	...
	$.post("https://wnStaff/enviarAnuncioInfo",JSON.stringify({
...
function FazerAnuncioAll(){
	...
	if($el.length) {
		$.post("http://wnStaff/fazeranuncioall",JSON.stringify({
...
function TrocarNome(){
	...
	if($el.length) {
		$.post("http://wnStaff/trocarnome",JSON.stringify({
...
function TrocarCarteira(){
	...
	if($el.length) {
		$.post("http://wnStaff/trocarcarteira",JSON.stringify({
			valor:valor,
...
function TrocarBanco(){
	...
	if($el.length) {
		console.log('teste')
		$.post("http://wnStaff/trocarbanco",JSON.stringify({
			valor:valor,
			tipo: $el.attr('data-tipo')
		}), (data) =>{ 
			if(data.retorno == 'done') {
...
function TrocarCelular(){
	...
	if($el.length) {
		$.post("http://wnStaff/trocarcelular",JSON.stringify({
...
function TrocarRegistro(){
	...
	if($el.length) {
		$.post("http://wnStaff/trocarregistro",JSON.stringify({
...
function VerInventario(verify){
    $.post("http://wnStaff/PegarInv",JSON.stringify({}),(data) => {
...
function removerItem(data){
	$.post("http://wnStaff/removerItem",JSON.stringify({
		item : data.dataset.item,
...
function VerGaragem(verify){
    $.post("http://wnStaff/PegarGaragem",JSON.stringify({}),(data) => {
...
function removerCarro(data){
	$.post("http://wnStaff/removerCarro",JSON.stringify({
		item : data.dataset.item,
	}), (data) =>{ 
		if(data.retorno == 'done') {
			LimparInputs()
...
function verBauCarro(data){
	$.post("http://wnStaff/verBauCarro",JSON.stringify({
...
function verBauCarroList(verify){
    $.post("http://wnStaff/verBauCarroList",JSON.stringify({}),(data) => {
		...
		carroBau.forEach((key,value) => {
			...
			$('.baucarro-box').prepend(`
				...
				<div class="item-baucarro">
					...
					<img src="${key.linkinventario}/${key.index}.png" alt="">
					<div class="quantidade-item">${key.amount}x</div>
					<div class="remove-baucarro" onclick="removerItemBauCarro(this)" data-item="${key.item}" data-quantidade="${key.amount}"><i class="fas fa-trash-alt"></i></div>
					<div class="barra1" style="background: ${actualPercent == 1 ? "#fc5858":colorPicker2(actualPercent)};">
						<div class="barra2" style="width: ${actualPercent == 1 ? "100":actualPercent}%; background: ${actualPercent == 1 ? "#fc5858":colorPicker(actualPercent)};"></div>
					</div>
				</div>
...
function removerItemBauCarro(data){
	$.post("http://wnStaff/removerItemBauCarro",JSON.stringify({
		item : data.dataset.item,
		quantidade : data.dataset.quantidade,
	}), (data) =>{ 
		if(data.retorno == 'done') {
...
function VerCasas(verify){
    $.post("http://wnStaff/PegarCasas",JSON.stringify({}),(data) => {
...
function removerCasa(data){
	$.post("http://wnStaff/removerCasa",JSON.stringify({
...
function verBauCasa(data){
	$.post("http://wnStaff/verBauCasa",JSON.stringify({
		casa : data.dataset.casa,
	}), (data) =>{ 
		if(data.retorno == 'done') {
			LimparInputs()
...
function verBauCasaList(verify){
    $.post("http://wnStaff/verBauCasaList",JSON.stringify({}),(data) => {
...
function removerItemBauCasa(data){
	$.post("http://wnStaff/removerItemBauCasa",JSON.stringify({
		item : data.dataset.item,
		quantidade : data.dataset.quantidade,
	}), (data) =>{ 
		if(data.retorno == 'done') {
...
function setDiscordId(data){
	...
	$.post("http://wnStaff/setDiscordId",JSON.stringify({
		id : id,
	}), (data) =>{ 
		if(data.retorno == 'done') {
			LimparInputs()
...
function VerEmpregos(verify){
    $.post("http://wnStaff/PegarEmpregos",JSON.stringify({}),(data) => {
...
function removerCargo(data){
	$.post("http://wnStaff/removerCargo",JSON.stringify({
...
function addemprego(data){
	$.post("http://wnStaff/addEmprego",JSON.stringify({}),(data) => {
...
function confirmarCargo(data){
	$.post("http://wnStaff/confirmaremprego",JSON.stringify({
		emprego : data.dataset.emprego,
		tipo : data.dataset.tipo,
...
function opcoesRapidas(tipo){
	$.post("http://wnStaff/opcoesRapidas",JSON.stringify({
		tipo : tipo,
	}), (data) =>{ 
		if(data.retorno == 'done') {
			LimparInputs()
...
function screenshot(){
	...
	$(".format").fadeIn(700);
	$.post("http://wnStaff/screenshot",JSON.stringify({
	}), (data) =>{ 
		if(data.retorno == 'done') {
			LimparInputs()
			Reload()
...
function EnviarMensagem(tipo){
	...
	$.post("http://wnStaff/enviarMensagem",JSON.stringify({
		mensagem : mensagem,
	}), (data) =>{ 
		if(data.retorno == 'done') {
			LimparInputs()
...
function ListaSkins(){
    $.post("http://wnStaff/skinsLista",JSON.stringify({}),(data) => {
...
function setarSkin(data){
	$.post("http://wnStaff/setarSkin",JSON.stringify({
...
function ListaBausfac(){
    $.post("http://wnStaff/bausfacLista",JSON.stringify({}),(data) => {
...
function verBauFac(data){
	$.post("http://wnStaff/RegisterBauFac",JSON.stringify({
...
function verBauFacList(verify){
    $.post("http://wnStaff/verbausfacLista",JSON.stringify({}),(data) => {
...
function removerItemBauFac(data){
	$.post("http://wnStaff/removerItemBauFac",JSON.stringify({
		item : data.dataset.item,
		quantidade : data.dataset.quantidade,
	}), (data) =>{ 
		if(data.retorno == 'done') {
...
function verGrousList(verify){
    $.post("http://wnStaff/verGrousList",JSON.stringify({}),(data) => {
...
function RegisterGroup(data){
	$.post("http://wnStaff/RegisterGroup",JSON.stringify({
...
function verPlayersGroup(verify){
    $.post("http://wnStaff/verPlayersGroup",JSON.stringify({}),(data) => {
...
function demitirGroups(data){
	$.post("http://wnStaff/demitirGroups",JSON.stringify({
		passaporte : data.dataset.passaporte,
		tipo : data.dataset.tipo,
	}), (data) =>{ 
		if(data.retorno == 'done') {
...
function createBan(){
	...
	if($el.length) {
		$.post("http://wnStaff/addBan",JSON.stringify({
			motivo:motivo
		}), (data) =>{ 
			if(data.retorno == 'done') {
				$(".aplicar-ban").fadeOut(500);
...
function createKick(){
	...
	if($el.length) {
		$.post("http://wnStaff/addKick",JSON.stringify({
			motivo:motivo
		}), (data) =>{ 
			if(data.retorno == 'done') {
				LimparInputs()
...
function createAdv(){
	...
	if($el.length) {
		$.post("http://wnStaff/addAdv",JSON.stringify({
			motivo:motivo
		}), (data) =>{ 
			if(data.retorno == 'done') {
				LimparInputs()
