		...
		if parseInt(#policeResult) <= 2 or os.time() < atmTimers then
			...
			for k,v in pairs(policeResult) do
				async(function()
					...
					TriggerClientEvent("NotifyPush",v,{ code = 20, title = "Caixa Eletrônico", x = coords["x"], y = coords["y"], z = coords["z"], criminal = "Alarme de segurança", time = os.date("%H:%M"), blipColor = 16 })
				end)
			end
...
function cRP.paymentSystems(locate)
	...
	if user_id then
		if os.time() < atmTimers then
			local randPrice = math.random(9500,13000)
