...
-- RegisterCommand('salariot',function(source,args,rawCommand)
--     vSERVER.receiveSalary()
-- end)  
-----------------------------------------------------------------------------------------------------------------------------------------
-- SEATSHUFFLE
	...
	while true do
		if energetic > 0 then
			...
			if energetic <= 0 or GetEntityHealth(PlayerPedId()) <= 101 then
				SetRunSprintMultiplierForPlayer(PlayerId(),20.0)
				energetic = 0
			end
		end
...
Citizen.CreateThread(function()
	while true do
		if ecstasy > 0 then
			...
			if ecstasy <= 1 or GetEntityHealth(PlayerPedId()) <= 101 then
				ecstasy = 0
...
local nots = false
RegisterCommand("bloqueartiro",function()

	if nots then
		if vSERVER.getPermissao() then
			TriggerEvent('Notify',"verde", 'Tiros Ativos')
			nots = false
		end
	else
		if vSERVER.getPermissao() then
			TriggerEvent('Notify',"verde", 'Tiros Desativados')
			nots = true
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedArmed(ped,6) and GetGameTimer() >= sprayTimers then
			...
			if IsPedShooting(ped) and not LocalPlayer.state.inLateGame and not LocalPlayer.state.inPvp and not nots then
