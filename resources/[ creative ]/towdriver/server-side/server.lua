...
AddEventHandler("towdriver:Call",function(source,vehName,vehPlate)
	...
	for k,v in pairs(userList) do
		async(function()
			TriggerClientEvent("NotifyPush",v,{ code = 51, imagem = "carro", title = "Registro de Veículo", criminal = "O Veiculo foi registrado "..vehicleName(vehName).." - "..vehPlate..".", x = coords["x"], y = coords["y"], z = coords["z"], vehicle = vehicleName(vehName).." - "..vehPlate, time = os.date("%H:%M"), blipColor = 33 })
		end)
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
