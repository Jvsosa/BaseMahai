...
-- CONFIG
-----------------------------------------------------------------------------------------------------------------------------------------
local IniciarServico = {23.1,6461.23,31.42,28.35} -- Onde se inicia o serviço e verifica a existência de um carro
local LocalDesmancharCarro = {15.64,6464.26,31.42,36.86} -- Onde deve haver o carro que será desmanchado para poder continuar o desmanche
local LocalFerramentas = {23.32,6464.79,31.42,45.36} -- Local onde 'pegará' as ferramentas
local AnuncioChassi = {37.05,6463.92,32.07,328.82} -- Onde finalizará a missão para entregar o chassi e receber dinheiro e itens
local Computador = {37.05,6463.92,32.07,328.82} -- Local onde ficará o computador de venda
---------------------------------------------------------------------
--VARIAVEIS
---------------------------------------------------------------------
local blips = {}
                                ...
                                if VehPermitido then 
                                    if CheckVehPermitido(nomeCarro) then
                                        if vSERVER.CheckItem() then
                                            ...
                                            TriggerEvent('Notify','vermelho','ERRO AO INICIAR O SERVIÇO! Você necessita de uma <b>Lock Pick</b> para iniciar o serviço.',10000)
                                        end
                                    else
