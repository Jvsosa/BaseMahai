...
local robberys = {
	...
	["46"] = {
		...
		["cops"] = 6,
		["payment"] = {
			{ "dollars2",500000,700000 }
		}
	},
	["47"] = {
		["coords"] = { 246.41,230.1,106.51 },
		["name"] = "Vinewood Vault",
		["type"] = "banks",
		["distance"] = 20.0,
		["cooldown"] = 360,
...
function cRP.AlertarPMs(x,y,z,name)
    local policeResult = vRP.getUsersByPermission("Police")
     for k,v in ipairs(policeResult) do
        ...
        if player and player ~= source then
            async(function()
                TriggerClientEvent("RB:AdicionarCDS",player,x,y,z)
                TriggerClientEvent("Notify",player,"azul","Houve um roubo em uma "..name.." e o alarme foi ativado, vá até o local!",5000)
            end)
        end
	end
end
		...
		if robberys[robberyId] then
			...
			if os.time() >= robberyAvailable[prev["type"]] then
				...
				if parseInt(#policeResult) >= parseInt(prev["cops"]) then
					...
					if parseInt(consultItem[1]) <= 0 then
					...
					if vRP.tryGetInventoryItem(user_id,consultItem[2],1) then
						...
						for k,v in pairs(policeResult) do
							async(function()
								TriggerClientEvent("NotifyPush",v,{ code = 31, imagem = "roubo", title = prev["name"], criminal = "Acabou de acontecer um roubo a uma "..prev["name"]..", va ate ao local.", x = prev["coords"][1], y = prev["coords"][2], z = prev["coords"][3], time = os.date("%H:%M"), blipColor = 22 })
...
function cRP.paymentRobbery(robberyId)
	...
	if user_id then
		...
		for k,v in pairs(robberys[robberyId]["payment"]) do
			...
			PerformHttpRequest("https://discord.com/api/webhooks/1124742474787729448/guoSDlcugZyE41GFACSV_2lcciOwaQHg840EwGfWO0BKuMyXsptSDw7ZouvN79-QYVdk", function(err, text, headers) end, 'POST', json.encode({
				embeds = {
					{     
						title = "**"..robberys[robberyId]["name"].." "..robberyId.."**",
						fields = {
							{ 
								name = "📝 Author:", 
							...
							{ 
								name = "💰 Valor:", 
								value = ""..parseInt(value).." Dollars Marcados",
							},
						}, 
						footer = { 
...
RegisterCommand('carregarroubos', function(source,args,rawCommand)
...
AddEventHandler("robberys:initJewelry",function()
	...
	if user_id then
		if os.time() >= jewelryCooldowns then
			local policeResult = vRP.numPermission("Police")
			if parseInt(#policeResult) >= 10 then
				...
				if vRP.tryGetInventoryItem(user_id,consultItem[2],1) then
					...
					for k,v in pairs(policeResult) do
						async(function()
							...
							TriggerClientEvent("NotifyPush",v,{ code = 31, imagem = "roubo", title = "Roubo a Lojinha", criminal = "Acabou de acontecer um roubo a uma Joalheria, va ate ao local.", x = -633.07, y = -238.7, z = 38.06, time = os.date("%H:%M"), blipColor = 22 })
							vRPC.playSound(v,"Beep_Green","DLC_HEIST_HACKING_SNAKE_SOUNDS")
						end)
					end
				end
