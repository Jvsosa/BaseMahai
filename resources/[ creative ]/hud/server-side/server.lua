...
cRP.getPerolas = function()
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		local identity = vRP.userIdentity(user_id)
		return vRP.userGemstone(identity["steam"])
	end
end
	...
	while true do
		...
		if GlobalState["Minutes"] >= 60 then
			...
			if GlobalState["Hours"] >= 24 then
				...
				if GlobalState["Weather"] == weatherList[randWeather] then
		...
		if wantedTimers[user_id] > os.time() then
			...
			if callTimers[user_id] <= os.time() and source > 0 then
				...
				for k,v in pairs(policeResult) do
					async(function()
						TriggerClientEvent("NotifyPush",v,{ code = 20, title = "Digitais Encontrada", x = coords["x"], y = coords["y"], z = coords["z"], criminal = "Alerta de procurado", time = os.date("%H:%M"), blipColor = 16 })
					end)
				end
			end
