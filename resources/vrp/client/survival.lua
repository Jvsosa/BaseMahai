...
AddEventHandler("vRP:playerActive",function(user_id,name)
	...
	SetDiscordAppId(1097392735998193715)
	SetDiscordRichPresenceAsset("512")
	SetRichPresence("#"..user_id.." "..name)
	SetDiscordRichPresenceAssetSmall("512")
	SetDiscordRichPresenceAssetText("Maui Beach")
	SetDiscordRichPresenceAssetSmallText("Maui Beach")
	SetDiscordRichPresenceAction(0,"Discord","https://discord.gg/maui")
...
-----------------------------------------------------------------------------------------------------------------------------------------
 Citizen.CreateThread(function()
 	while true do
 		...
 		if LocalPlayer["state"]["Active"] then
 			local ped = PlayerPedId()
 			if GetEntityHealth(ped) <= 101 and not LocalPlayer.state.inLateGame and not LocalPlayer.state.inPvp then
...
function tvRP.respawnPlayer()
	...
	DoScreenFadeOut(0)
	SetEntityCoordsNoOffset(PlayerPedId(),-1324.64,-1515.45,4.43,false,false,false,true)
	SendNUIMessage({ death = false })
	Citizen.Wait(1000)
	DoScreenFadeIn(1000)
end

function tvRP.respawnPlayer2()
	timeDeath = 300
	deathStatus = false
	...
	DoScreenFadeOut(0)
	SetEntityCoordsNoOffset(PlayerPedId(),2523.51,-355.25,94.12,false,false,false,true)
	SendNUIMessage({ death = false })
	Citizen.Wait(1000)
	DoScreenFadeIn(1000)
end
