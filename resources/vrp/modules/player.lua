...
AddEventHandler("playerConnect",function(user_id,source)
	...
	if dataTable then
		...
		local colete = dataTable["armour"] or 0
		...
		TriggerClientEvent("nation_skinshop:apply",source,vRP.userData(user_id,"Clothings"))
...
RegisterCommand("gg",function(source,args,rawCommand)
	...
	if user_id and vRPC.checkDeath(source) then
		...
		-- if not vRP.hasPermission(user_id,"vipsilver") or not vRP.hasPermission(user_id,"vipgold") or not vRP.hasPermission(user_id,"vipplatinum") or not vRP.hasPermission(user_id,"vipdiamond") or not vRP.hasPermission(user_id,"vipmaui") or not vRP.hasPermission(user_id,"Admin") then
		-- 	vRP.resetWeight(user_id)
		-- end
