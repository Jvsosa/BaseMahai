...
-- SETPREMIUMDELUXE
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.setPremiumDeluxe(user_id)
	vRP.execute("accounts/setPremium",{ steam = vRP.userInfos[user_id]["steam"], premium = os.time() + 31536000, priority = 100 })

	if vRP.userInfos[user_id] then
		vRP.userInfos[user_id]["premium"] = parseInt(os.time() + 31536000)
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- UPGRADEPREMIUMDELUXE
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.upgradePremiumDeluxe(user_id)
	vRP.execute("accounts/updatePremium",{ steam = vRP.userInfos[user_id]["steam"] })

	if vRP.userInfos[user_id] then
		vRP.userInfos[user_id]["premium"] = vRP.userInfos[user_id]["premium"] + 31536000
	end
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- USERPREMIUM
