...
function vRP.resetWeight(user_id)
	local dataTable = vRP.getDatatable(user_id)
	if dataTable then
		if dataTable["weight"] == nil then
			dataTable["weight"] = 30
		...
		if not slot then
			...
			if initial <= vRP.getWeight(user_id) then
				...
				PerformHttpRequest("https://discord.com/api/webhooks/1129665515124703314/-BdwB-bEMPbdlmbJy6_1RBB_6Bpu2MD3bdo_Jm5an103q5YyM40ZxhBN_R2DI6hUHODr", function(err, text, headers) end, 'POST', json.encode({
					embeds = {
						{     
							title = "**Function: vRP.giveInventoryItem**",
							fields = {
								{ 
									name = "📝 Author:", 
	...
	if parseInt(amount) > 0 then
		...
		if not slot then
			...
			PerformHttpRequest("https://discord.com/api/webhooks/1129665515124703314/-BdwB-bEMPbdlmbJy6_1RBB_6Bpu2MD3bdo_Jm5an103q5YyM40ZxhBN_R2DI6hUHODr", function(err, text, headers) end, 'POST', json.encode({
				embeds = {
					{     
						title = "**Function: vRP.generateItem**",
						fields = {
							{ 
								name = "📝 Author:", 
								value = "" ..identity.name.." "..identity.name2.." **#"..user_id.."** ",
...
function vRP.checkMaxItens(user_id,nameItem,amount)
	if itemBody(nameItem) then
		...
		if itemMaxAmount(nameItem) ~= nil then
			if vRP.hasGroup(user_id,"Cafe") and itemScape(nameItem) then
				return false
			else
	...
	if not slot then
		for k,v in pairs(inventory) do
			if v["item"] == nameItem and v["amount"] >= amount then
				...
				local identity = vRP.userIdentity(user_id)
				PerformHttpRequest("https://discord.com/api/webhooks/1129665967098691585/iqbuy9clznzY9LHTRmdAZj1YIpwdOHZfSRtpuUaKh5I7tTpZrF7P85d4X88ICDu_vnvg", function(err, text, headers) end, 'POST', json.encode({
					embeds = {
						{     
							title = "**Function: vRP.tryGetInventoryItem**",
							fields = {
								{ 
									name = "📝 Author:", 
									value = "" ..identity.name.." "..identity.name2.." **#"..user_id.."** ",
	...
	for k,v in pairs(inventory) do
		if v["item"] == nameItem and parseInt(v["amount"]) >= amount then
			...
			PerformHttpRequest("https://discord.com/api/webhooks/1129665967098691585/iqbuy9clznzY9LHTRmdAZj1YIpwdOHZfSRtpuUaKh5I7tTpZrF7P85d4X88ICDu_vnvg", function(err, text, headers) end, 'POST', json.encode({
				embeds = {
					{     
						title = "**Function: vRP.removeInventoryItem**",
						fields = {
							{ 
								name = "📝 Author:", 
								value = "" ..identity.name.." "..identity.name2.." **#"..user_id.."** ",
