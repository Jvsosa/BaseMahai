...
-- CLEARTABLES
-----------------------------------------------------------------------------------------------------------------------------------------
vRP.prepare("/playerdata","DELETE FROM playerdata WHERE dvalue = '[]' OR dvalue = '{}'")
vRP.prepare("/entitydata","DELETE FROM entitydata WHERE dvalue = '[]' OR dvalue = '{}'")
-- vRP.prepare("/cleanBanks","DELETE FROM smartphone_bank WHERE (DATEDIFF(CURRENT_DATE,data) >= 7)")
vRP.prepare("/cleanPremium","UPDATE accounts SET premium = '0', priority = '0' WHERE UNIX_TIMESTAMP() >= premium")
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADCLEANERS
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
	vRP.execute("/playerdata")
	vRP.execute("/entitydata")
	-- vRP.execute("/cleanBanks")
	vRP.execute("/cleanPremium")
